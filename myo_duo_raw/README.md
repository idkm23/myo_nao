This project is a fork of dzhu's awesome myo_raw. It is modified to run with two Myo armbands running on two USB dongles on the same PC. This allows the user to keep the same 200Hz sampling rate of EMG and the 50Hz sampling rate of the IMUs while using two armbands simultaneously. 

<b>NOTE:</b> This was only tested on Mac OSX, not on Windows or Linux. You are welcome to modify the project accordingly or to try running them on those operating systems yourself.

To run this project with two myos, simply plug in both dongles to your computer and run dual_myo.py. Make sure your myos are turned on when the program is running. 
