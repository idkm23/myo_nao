...
class MyoRaw(object):
    ...
    def __init__(self, tty=None, identification=None):
        if tty is None:
            tty = self.detect_tty()
        if tty is None:
    ...
    def connect(self):
        ...
        def handle_data(p):
            ...
            if attr == 0x27:
                ...
                emg = vals[:8]
                moving = vals[8]
                self.on_emg(current_milli_time(), emg, moving, self.id)
            elif attr == 0x1c:
                vals = unpack('10h', pay)
                quat = vals[:4]
                acc = vals[4:7]
                gyro = vals[7:10]
                self.on_imu(current_milli_time(), quat, acc, gyro, self.id)
            elif attr == 0x23:
                typ, val, xdir = unpack('3B', pay)

                if typ == 1: # on arm
                    self.on_arm(Arm(val), XDirection(xdir), self.id)
                elif typ == 2: # removed from arm
                    self.on_arm(Arm.UNKNOWN, XDirection.UNKNOWN, self.id)
                elif typ == 3: # pose
                    self.on_pose(Pose(val), self.id)
            else:
    ...
    def on_emg(self, timestamp, emg, moving, id):
        for h in self.emg_handlers:
            h(timestamp, emg, moving, id)

    def on_imu(self, timestamp, quat, acc, gyro, id):
        for h in self.imu_handlers:
            h(timestamp, quat, acc, gyro, id)

    def on_pose(self, p, id):
        for h in self.pose_handlers:
            h(p, id)

    def on_arm(self, arm, xdir, id):
...
if __name__ == '__main__':
    ...
    def proc_emg(timestamp, emg, moving, times=[]):
        if HAVE_PYGAME:
